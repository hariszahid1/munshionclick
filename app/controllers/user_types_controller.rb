class UserTypesController < ApplicationController
  ...
  def index
    ...
    if params[:q].present?
      ...
      @code = params[:q][:code]
      @discount_by_percentage = params[:q][:discount_by_percentage]
      @discount_by_amount = params[:q][:discount_by_amount]
      @comment = params[:q][:comment]
    end
    @user_types = @q.result(distinct: true).page(params[:page])
  end
