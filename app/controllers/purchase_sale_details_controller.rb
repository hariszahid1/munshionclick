class PurchaseSaleDetailsController < ApplicationController
        ...
        if params[:sale_submit].present?
          ...
          if staff
            ...
            if user_name
              ...
              @products_sale_price_items = PurchaseSaleDetail.joins(purchase_sale_items: :item).where(transaction_type: "Sale").where('purchase_sale_items.item_id':item_id).where(sys_user_id: sys_user_id,  'purchase_sale_items.created_at': @created_at_gteq.to_date.beginning_of_day..@created_at_lteq.to_date.end_of_day,transaction_type: "Sale").group(:title).average('purchase_sale_items.sale_price')
              @products_sale_items = PurchaseSaleDetail.joins(purchase_sale_items: :item).where(transaction_type: "Sale").where('purchase_sale_items.item_id':item_id).where(sys_user_id: sys_user_id,  'purchase_sale_items.created_at': @created_at_gteq.to_date.beginning_of_day..@created_at_lteq.to_date.end_of_day,transaction_type: "Sale").group(:title).sum(:total_sale_price)
              @products_count_items = PurchaseSaleDetail.joins(purchase_sale_items: :item).where(transaction_type: "Sale").where('purchase_sale_items.item_id':item_id).where(sys_user_id: sys_user_id, 'purchase_sale_items.created_at': @created_at_gteq.to_date.beginning_of_day..@created_at_lteq.to_date.end_of_day,transaction_type: "Sale").group(:title).sum('purchase_sale_items.quantity')
              @products_sale_total_items = PurchaseSaleDetail.joins(purchase_sale_items: :item).where(transaction_type: "Sale").where('purchase_sale_items.item_id':item_id).where(sys_user_id: sys_user_id, 'purchase_sale_items.created_at': @created_at_gteq.to_date.beginning_of_day..@created_at_lteq.to_date.end_of_day,transaction_type: "Sale").sum(:total_sale_price)
  ...
  def show
    ...
    if @pos_setting.sys_type=="industry" || @pos_setting.sys_type =="HousingScheme"
      if @pos_setting&.extra_settings.present?
        if @pos_setting&.extra_settings['ghouse5'].present? || @pos_setting&.extra_settings['al_fatir'].present?
          respond_to do |format|
            format.pdf do
